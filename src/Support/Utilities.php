...
class Utilities {
	...
	public static function convertCsvToExcel(string $csvPath, ?string $excelPath=NULL, ?string $delimiter=NULL): string {
		...
		if (!in_array(mime_content_type($csvPath), ['text/csv', 'text/plain'])) {
	...
	public static function customErrorHandler(int $errno, string $errstr, string $errfile, int $errline, ?array $errcontext=NULL): void {
		...
		// list all errors in framework log
			...
			if (Options::get('show_log')) {
				...
				ErrorLog::snapshot('Failure in ' . $errfile . ' line ' . $errline . ' with error ' . $errstr);
	...
	public static function customExceptionHandler(\Throwable $e): void {
		...
		// list all errors in framework log
			...
			$fullDescription = get_class($e) . ' in ' . $e->getFile() . ' line ' . $e->getLine() . ' with error ' . $e->getMessage();

			ErrorLog::snapshot($fullDescription);
	...
	public static function fatalErrorHandler(): void {
		...
		// start to show detailed error in log event
		Logger::error('Fatal error [' . $error['type'] . ']: ' . $error['message'] . ' in ' . $error['file'] . ' line ' . $error['line']);
		...
		ErrorLog::snapshot('Fatal error [' . $error['type'] . ']: ' . $error['message'] . ' in ' . $error['file'] . ' line ' . $error['line']);

		if (Config::get('SENTRY_DSN')) {
			\Sentry\captureLastError();
		}

		BugSnag::error($error['type'] ?? 'FatalError', $error['message'] ?? 'Unknown error');
	...
	public static function getActiveRecordClasses(): array {
		...
		$checkFolder(APPLICATION_PATH . '/' . PAIR_FOLDER . '/Support', 'Pair\\Support\\');
