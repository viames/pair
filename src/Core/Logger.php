...
class Logger {
	...
	private function __construct() {
		...
		if (Config::get('PAIR_LOGGER_EMAIL_THRESHOLD')) {
			...
			if ($this->emailThreshold < 1 or $this->emailThreshold > 8) {
				throw new LoggerException('Invalid PAIR_LOGGER_EMAIL_THRESHOLD value in configuration file.');
			}
		}
		...
		if (Config::get('PAIR_LOGGER_TELEGRAM_THRESHOLD')) {
			...
			if ($this->telegramThreshold < 1 or $this->telegramThreshold > 8) {
				throw new LoggerException('Invalid PAIR_LOGGER_TELEGRAM_THRESHOLD value in configuration file.');
			}
		}
	...
	private function checkMailer(): void {
		...
		if (!$this->mailer) {
			throw new LoggerException('Mailer not configured.');
		}
	...
	public static function errorHandler(int $errno, string $errstr, string $errfile, int $errline, ?array $errcontext=NULL): void {
		...
		// list all errors in framework log
			...
			$app->style = '500';
			$app->pageContent = 'development' == $app->getEnvironment()
				? '“' . $errstr .  '” in ' . $file . ' line ' . $errline
				: 'Internal Server Error';

			http_response_code(500);

			// start to show detailed error in log event
			Logger::notice('Debug backtrace for “' . $errstr .  '” in ' . $file . ' line ' . $errline, 'error');

			foreach ($backtrace as $event) {
				Logger::notice($event);
			}

			Logger::notice('Debug backtrace finished');

		}

		// register error in database and notify
		$self = self::getInstance();
		$self->handle('Failure in ' . $file . ' line ' . $errline . ' with error ' . $errstr, self::ERROR);
	...
	public static function exceptionHandler(\Throwable $e): void {
		...
		// list all errors in framework log
			...
			$fullDescription = 'Uncaught ' . get_class($e) . ($e->getCode() ? ' ' . $e->getCode() : '') . ' in ' . $file . ' line ' . $e->getLine() . ': ' . $e->getMessage();

			// register error in database and notify
			$self = self::getInstance();
			$self->handle($fullDescription, self::ERROR);
	...
	public function setEmailThreshold(int $threshold): void {
		...
		if ($threshold < 1 or $threshold > 8) {
			throw new LoggerException('Invalid E-mail error threshold value', ErrorCodes::LOGGER_FAILURE);
		}
	...
	public function setTelegramThreshold(int $threshold): void {
		...
		if ($threshold < 1 or $threshold > 8) {
			throw new LoggerException('Invalid Telegram error threshold value', ErrorCodes::LOGGER_FAILURE);
		}
	...
	public static function shutdownHandler(): void {
		...
		// start to show detailed error in log event
		Logger::notice('Fatal error [' . $error['type'] . ']: ' . $error['message'] . ' in ' . $error['file'] . ' line ' . $error['line'], 'error');
		...
		// register error in database and notify
		$self = self::getInstance();
		$self->handle('Fatal error [' . $error['type'] . ']: ' . $error['message'] . ' in ' . $error['file'] . ' line ' . $error['line'], self::ERROR);
	...
	public static function snapshot(string $description, ?int $level=NULL): void {
		...
		$errorLog->path			= substr($router->url,1);
		...
		Logger::notice($description, 'error', 'Snapshot taken');
