...
abstract class View {
	...
	final public function display(?string $name=NULL): void {
		...
		try {
			...
			$this->logError('Error in render() method: ' . $e->getMessage());
		}
	...
	protected function raiseError(ActiveRecord $object): void {
		...
		// send the error as a modal message
		$this->app->modal('Error', $message, 'error');

		// after the message has been queued, store the error data
		ErrorLog::snapshot('Failure in ' . \get_class($object) . ' class');
